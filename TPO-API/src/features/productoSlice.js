...
export const fetchProductosDestacados = createAsyncThunk('productos/fetchProductosDestacados', async () => {
  const response = await fetch(urlServer2 + 'catalogo');
  if (!response.ok) {
    throw new Error('Error al obtener los productos.');
  }
...
export const fetchJuegos = createAsyncThunk('productos/fetchJuegos', async () => {
  const response = await fetch(urlServer2 + 'catalogo');
  if (!response.ok) {
    throw new Error('Error al obtener los productos.');
  }
